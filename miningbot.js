...
function main() {
    ...
    while (true)  {
        var rock = findRock();
        //jos kiviä ei löydy lähettää error viestin
        if (rock == false){
            rotateCamera();
...
function testScreenCapture() {
    ...
    var pixel_color = img.colorAt(308, 289);
    console.log(pixel_color);
    var pixel_color = img.colorAt(132, 261);
    console.log(pixel_color);
    var pixel_color = img.colorAt(254, 350);
    console.log(pixel_color);
}
function findRock() {
...
//löytää värit
    ...
    for (var i = 0; i < 3000; i++) {
        ...
        if (rock_colors.includes(sample_color)) {
            var screen_x = random_x + x;
            var screen_y = random_y + y;

            console.log("Löydettiin rautaa koordinaateista: " + screen_x + ", " + screen_y + " väri on " + sample_color);
